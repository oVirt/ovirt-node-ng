...
UPDATESIMG=${UPDATESIMG:-updates.img}
...
cond_curl "$UPDATESIMG" "http://jenkins.ovirt.org/job/ovirt-node-ng_${BRANCH}_build-artifacts-fc22-x86_64/lastStableBuild/artifact/exported-artifacts/updates.img"
...
{
  set -e
  PRODUCTIMG=$PRODUCTIMG UPDATESIMG=$UPDATESIMG bash $DERVICEBOOTISOSCRIPT "$BOOTISO" "$SQUASHFS" "$NEWISO"
